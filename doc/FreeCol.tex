...
\title{FreeCol Documentation\\User Guide for Version v0.7.2}
\maketitle{}
...
which a Java Virtual Machine compatible with Sun Java 5 or 6 is
...
\href{http://java.sun.com/}{here}, but not with projects based on GNU
Classpath or OpenJDK, as some font handling classes used by FreeCol
have not yet been implemented. This is expected to change in the near
future, however. FreeCol is known to run on recent versions of Windows
and Linux, and on MacOSX to some extent. If you are using Linux, using
Java 6 is recommended, as its font rendering is much better. If you
...
includes translations into Breton, Finnish, French, German, Hungarian,
Italian, (Brazilian) Portuguese, Russian, Spanish and Swedish, some of
which are not very complete, however. Java will automatically select
the translation for your locale, if available, and English
otherwise. If you should wish to select a different language, or if
language selection fails, you can choose a different language from the
...
size and terrain of a randomly generated map. FreeCol includes a large
map of the Americas, which can be selected by clicking on the map
icon.
...
\begin{itemize}
\item The language\index{language} to use. Some languages are spoken in
...
only available if the unit is carrying units and is in a colony or in
Europe, or if it is carrying goods. You can unload the goods anywhere,
but if you are not in Europe or in a colony, the goods will be lost.
You can use this feature to dump unwanted cargo\index{dump cargo} in
order to avoid the \hyperlink{Cargo Penalty}{cargo penalty}.
...
In the original Colonization game, a unit used up all movement points
when entering a colony. In FreeCol, this is not the case --- a unit
can enter a colony just like any other tile. If the unit is placed in
a building, or on a colony tile, or if a carrier is loaded or
unloaded, however, it will lose all its movement points.
...
If a ship has docked at the European port can drag and drop units
...
Going to America section of the Europe panel.
...
wagon train
\item Open the warehouse dialog in able to change the export and
...
In the case of open land, plowing increases the production
...
that should not be kept in reserve.
...
yourself by placing a skilled unit in one of these buildings. Note
that the Master Sugar Planter, the Master Cotton Planter, the Master
Tobacco Planter, the Expert Fur Trapper, the Master Distiller, the
Master Weaver, the Master Tobacconist, the Master Blacksmith and the
Master Gunsmith all require at least a \hyperlink{College}{College},
while the Elder Statesman, the Firebrand Preacher and the Jesuit
Missionary even require a \hyperlink{University}{University} to teach
their profession.
...
\item \hypertarget{Veteran Bonus}{Veteran Bonus}: the offensive and
  defensive power of veteran units is increased by one.
...
In the future, the European enemies of your Monarch may support your
effort if you generate a sufficient number of Liberty Bells after the
War of Independence has begun.
