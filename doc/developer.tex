...
\item Change the \verb$FREECOL_VERSION$ constant to match the release version in
...
\item Add a news item to our website.

\item Remove the download button from the previous release news item.

\item Post release messages on our mailing lists: developers, translators
  and users. Include information on savegame compatibility with previous versions 
  on all messages.
  
\item Post release message to forum FreeCol. Include information on savegame compatibility with previous versions,
  as well as information on how to post bug reports regarding the release in the bug tracker.
...
\item Update JavaDoc and User Documentation on
