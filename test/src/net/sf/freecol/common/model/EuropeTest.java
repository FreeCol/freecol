...
public class EuropeTest extends FreeColTestCase {
    ...
    public void testMissionary() {
        ...
        assertTrue("Colonist should allow dressing missionaries",
            colonist.hasAbility(Ability.DRESS_MISSIONARY));
    }
    ...
    public void testRecruitPriceCalculation() {
        ...
        // Initial state: 0 immigration, price should be at maximum (base recruit price = 200)
        dutch.setImmigration(0);
        int initialPrice = amsterdam.getCurrentRecruitPrice();
        assertEquals(200, initialPrice);

        // As immigration increases towards required, price should drop proportionally
        dutch.setImmigration(100);
        int midPrice = amsterdam.getCurrentRecruitPrice();
        assertTrue("Recruit price should drop as immigration increases", midPrice < initialPrice);

        dutch.setImmigration(190);
        int highImmigrationPrice = amsterdam.getCurrentRecruitPrice();
        assertTrue("Recruit price should drop further closer to immigration required", highImmigrationPrice < midPrice);
    }
