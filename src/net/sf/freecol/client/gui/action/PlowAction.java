...
public class PlowAction extends MapboardAction {
    ...
    private void updateValues(boolean p) {
        ...
        if (plow) {
            putValue(BUTTON_IMAGE, getFreeColClient().getImageLibrary().getUnitButtonImageIcon(ImageLibrary.UNIT_BUTTON_PLOW, 0));
            putValue(BUTTON_ROLLOVER_IMAGE, getFreeColClient().getImageLibrary().getUnitButtonImageIcon(ImageLibrary.UNIT_BUTTON_PLOW, 1));
            putValue(BUTTON_PRESSED_IMAGE, getFreeColClient().getImageLibrary().getUnitButtonImageIcon(ImageLibrary.UNIT_BUTTON_PLOW, 2));
            putValue(BUTTON_DISABLED_IMAGE, getFreeColClient().getImageLibrary().getUnitButtonImageIcon(ImageLibrary.UNIT_BUTTON_PLOW, 3));
            putValue(NAME, Messages.message("unit.state.5"));
        } else {
            putValue(BUTTON_IMAGE, getFreeColClient().getImageLibrary().getUnitButtonImageIcon(ImageLibrary.UNIT_BUTTON_CLEAR, 0));
            putValue(BUTTON_ROLLOVER_IMAGE, getFreeColClient().getImageLibrary().getUnitButtonImageIcon(ImageLibrary.UNIT_BUTTON_CLEAR, 1));
            putValue(BUTTON_PRESSED_IMAGE, getFreeColClient().getImageLibrary().getUnitButtonImageIcon(ImageLibrary.UNIT_BUTTON_CLEAR, 2));
            putValue(BUTTON_DISABLED_IMAGE, getFreeColClient().getImageLibrary().getUnitButtonImageIcon(ImageLibrary.UNIT_BUTTON_CLEAR, 3));
     ...
     * 
     * @return <code>false</code> if there is no active unit or if
     *      the unit cannot plow/clear forest.
     */
