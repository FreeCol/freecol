...
public final class GoodsLabel extends JLabel {// implements ActionListener {
     ...
     */
    /*
     * isn't actually used public void actionPerformed(ActionEvent event) {
     * String command = event.getActionCommand(); try { switch
     * (Integer.valueOf(command).intValue()) { default: logger.warning("Invalid
     * action"); }
     * setIcon(parent.getImageProvider().getGoodsImageIcon(goods.getType()));
     * repaint(0, 0, getWidth(), getHeight());
     *  // TODO: Refresh the gold label when goods have prices.
     * //goldLabel.repaint(0, 0, goldLabel.getWidth(), goldLabel.getHeight()); }
     * catch (NumberFormatException e) { logger.warning("Invalid action
     * number"); } }
     */
}
