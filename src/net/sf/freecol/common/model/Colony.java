...
public final class Colony extends Settlement implements Location, Nameable {
	 ...
	 * @param tile
	 *            The location of the <code>Colony</code>.
	 * @param initializeWorkLocations
	 *            If <code>true</code> then all standard
	 *            {@link WorkLocation work locations} will be added.
	 */
	public Colony(Game game, Player owner, String name, Tile tile,
			boolean initializeWorkLocations) {
		super(game, owner, tile);

		Iterator<Position> exploreIt = getGame().getMap().getCircleIterator(
				getTile().getPosition(), true, getLineOfSight());
		...
		if (initializeWorkLocations) {
			...
			for (int type = 0; type < numberOfTypes; type++) {
				BuildingType buildingType = FreeCol.specification
						.buildingType(type);
				if (buildingType.level(0).hammersRequired > 0) {
					workLocations.add(new Building(game, this, type,
							Building.NOT_BUILT));
				} else {
					workLocations.add(new Building(game, this, type,
							Building.HOUSE));
	...
	public void updatePopulation() {
		if (getUnitCount() >= 3
				&& getOwner().hasFather(FoundingFather.LA_SALLE)) {
	...
	public void setOwner(Player owner) {
		...
		while (unitIterator.hasNext()) {
			...
			if (unit.getLocation() instanceof ColonyTile) {
				((ColonyTile) unit.getLocation()).getWorkTile().setNationOwner(
						owner.getNation());
			}
		}
	 ...
	 * 
	 * @param t
	 *            The <code>Tile</code> to get the <code>ColonyTile</code>
	 *            for.
	...
	public void add(Locatable locatable) {
		if (locatable instanceof Unit) {
			...
			logger
					.warning("Tried to add an unrecognized 'Locatable' to a 'Colony'.");
		}
	}
	...
	public void remove(Locatable locatable) {
		if (locatable instanceof Unit) {
			...
			logger
					.warning("Tried to remove an unrecognized 'Locatable' from a 'Colony'.");
		}
	}
	 ...
	 * 
	 * @param type
	 *            The type of Goods to remove from this container.
	 * @param amount
	 *            The amount of Goods to remove from this container.
	 */
	 ...
	 * 
	 * @param attacker
	 *            The target that would be attacking this colony.
	...
	public Unit getDefendingUnit() {
		if (!getOwner().hasFather(FoundingFather.PAUL_REVERE)) {
			// Paul Revere is not a founding father, no need to go on.
			return null;
		}

		if (getUnitList().size() == 0) {
			// Noone to defend the colony.
			return null;
		}

		// Get the unit, arm and mount it and return it.
		Unit defender = getUnitList().get(0);
	 ...
	 * 
	 * @param unit
	 *            The <code>Unit</code> to find a vacant
	...
	public ColonyTile getVacantColonyTileFor(Unit unit, int goodsType) {
		...
		while (colonyTileIterator.hasNext()) {
			...
			if (colonyTile.canAdd(unit)) {
				...
				if (ourLand
						&& unit.getFarmedPotential(goodsType, colonyTile
								.getWorkTile()) > highestProduction) {
					highestProduction = unit.getFarmedPotential(goodsType,
	 ...
	 * 
	 * @param unit
	 *            The <code>Unit</code> to find the highest possible
	...
	public int getPotentialHorseProduction() {
		...
		int maxSpace = Math.max(0, getWarehouseCapacity()
				- getGoodsCount(Goods.HORSES));
	...
	private void checkBuildingComplete() {
		...
		if (getCurrentlyBuilding() >= Colony.BUILDING_UNIT_ADDITION) {
			...
			if (canBuildUnit(unitType)
					&& Unit.getNextHammers(unitType) <= getHammers()
					&& Unit.getNextHammers(unitType) != -1) {
				if (Unit.getNextTools(unitType) <= getGoodsCount(Goods.TOOLS)) {
					Unit unit = getGame().getModelController().createUnit(
							getID() + "buildUnit", getTile(), getOwner(),
							unitType);
					hammers = 0;
			...
			int hammersRequired = getBuilding(currentlyBuilding)
					.getNextHammers();
				...
				if (getGoodsCount(Goods.TOOLS) >= toolsRequired) {
					if (!getBuilding(currentlyBuilding).canBuildNext()) {
						throw new IllegalStateException(
								"Cannot build the selected building.");
					}
					if (toolsRequired > 0) {
						removeGoods(Goods.TOOLS, toolsRequired);
					}
					hammers = 0;
					getBuilding(currentlyBuilding).setLevel(
							getBuilding(currentlyBuilding).getLevel() + 1);
	...
	public void payForBuilding() {
		...
		if (getCurrentlyBuilding() >= Colony.BUILDING_UNIT_ADDITION) {
			...
			toolsRemaining = Math.max(getBuilding(currentlyBuilding)
					.getNextTools()
					- getGoodsCount(Goods.TOOLS), 0);
			hammers = Math.max(getBuilding(currentlyBuilding).getNextHammers(),
					hammers);
		}
		...
		if (hammersRemaining > 0) {
			getOwner().modifyGold(
					-hammersRemaining
							* getGameOptions().getInteger(
									GameOptions.HAMMER_PRICE));
		}
		if (toolsRemaining > 0) {
	 ...
	 * 
	 * @param defender
	 *            The <code>Unit</code> defending against bombardment.
	 * @param result
	...
	public void newTurn() {
		...
		if (eat > food) {
			...
			if (eat > getFoodProduction()
					&& (food - eat) / (eat - getFoodProduction()) <= 3) {
				addModelMessage(
						this,
						...
						new String[][] {
								...
								{
										"%number%",
										Integer.toString((food - eat)
												/ (eat - getFoodProduction())) } },
						ModelMessage.WARNING);
			}
		}
		...
		if (getGoodsCount(Goods.FOOD) >= 200) {
			Unit u = getGame().getModelController().createUnit(
					getID() + "newTurn200food", getTile(), getOwner(),
					Unit.FREE_COLONIST);
			removeGoods(Goods.FOOD, 200);
			addModelMessage(this, "model.colony.newColonist",
					new String[][] { { "%colony%", getName() } },
					ModelMessage.UNIT_ADDED);
			logger.info("New colonist created in " + getName() + " with ID="
					+ u.getID());
		}
		...
		goodsContainer.cleanAndReport(getWarehouseCapacity(), getLowLevel(),
				getHighLevel());
		...
		if (sonsOfLiberty == 100) {
			...
			if (oldSonsOfLiberty < 100) {
				addModelMessage(this, "model.colony.SoL100", new String[][] { {
						"%colony%", getName() } },
						ModelMessage.SONS_OF_LIBERTY, new Goods(Goods.BELLS));
			}
		} else {
			...
			if (tories > veryBadGovernment) {
				...
				if (oldTories <= veryBadGovernment) {
					...
					addModelMessage(this, "model.colony.veryBadGovernment",
							new String[][] { { "%colony%", getName() } },
							ModelMessage.GOVERNMENT_EFFICIENCY, new Goods(
									Goods.BELLS));
				}
	...
	protected void toXMLImpl(XMLStreamWriter out, Player player,
		...
		if (showAll || player == getOwner()) {
			...
			out
					.writeAttribute("sonsOfLiberty", Integer
							.toString(sonsOfLiberty));
			out.writeAttribute("oldSonsOfLiberty", Integer
					.toString(oldSonsOfLiberty));
			out.writeAttribute("tories", Integer.toString(tories));
			out.writeAttribute("oldTories", Integer.toString(oldTories));
			out.writeAttribute("productionBonus", Integer
					.toString(productionBonus));
			out.writeAttribute("currentlyBuilding", Integer
					.toString(currentlyBuilding));
			...
			while (workLocationIterator.hasNext()) {
				((FreeColGameObject) workLocationIterator.next()).toXML(out,
						player, showAll, toSavedGame);
			}
		} else {
			out.writeAttribute("unitCount", Integer.toString(getUnitCount()));
			getBuilding(Building.STOCKADE).toXML(out, player, showAll,
					toSavedGame);
		}
	...
	protected void readFromXMLImpl(XMLStreamReader in)
		...
		tile = (Tile) getGame().getFreeColGameObject(
				in.getAttributeValue(null, "tile"));
		if (tile == null) {
		...
		final String sonsOfLibertyStr = in.getAttributeValue(null,
				"sonsOfLiberty");
		...
		final String oldSonsOfLibertyStr = in.getAttributeValue(null,
				"oldSonsOfLiberty");
		...
		final String productionBonusStr = in.getAttributeValue(null,
				"productionBonus");
		...
		final String currentlyBuildingStr = in.getAttributeValue(null,
				"currentlyBuilding");
		...
		while (in.nextTag() != XMLStreamConstants.END_ELEMENT) {
			if (in.getLocalName().equals(ColonyTile.getXMLElementTagName())) {
				ColonyTile ct = (ColonyTile) getGame().getFreeColGameObject(
			...
			} else if (in.getLocalName()
					.equals(Building.getXMLElementTagName())) {
				Building b = (Building) getGame().getFreeColGameObject(
						in.getAttributeValue(null, "ID"));
				...
				GoodsContainer gc = (GoodsContainer) getGame()
						.getFreeColGameObject(in.getAttributeValue(null, "ID"));
				...
				exportLevel = readFromArrayElement("exportLevel", in,
						new int[0]);
			}
		}
	}
